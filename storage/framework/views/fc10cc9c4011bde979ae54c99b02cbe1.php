<?php $__env->startSection('title', 'Bulk Upload Status'); ?>
    ...
    <div class="row justify-content-center">
        <div class="col-md-10">
            <div class="d-flex align-items-center mb-4">
                <a href="<?php echo e(route('customer.services.bulk-upload')); ?>" class="btn btn-outline-secondary me-3">
            ...
            <div class="card mb-4">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <div class="d-flex align-items-center">
                        <?php switch($temporaryUpload->status):
                            case ('pending'): ?>
                                <span class="badge bg-warning me-2">Pending</span>
                                <form action="<?php echo e(route('customer.services.delete-bulk-upload', $temporaryUpload->id)); ?>" method="POST"
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6">
                            <p><strong>File Name:</strong> <?php echo e($temporaryUpload->original_filename); ?></p>
                            <p><strong>Upload Date:</strong> <?php echo e($temporaryUpload->created_at->format('M d, Y H:i:s')); ?></p>
                        </div>
                        <div class="col-md-6">
                            ...
                            <?php if($temporaryUpload->notes): ?>
                                <p><strong>Notes:</strong> <?php echo e($temporaryUpload->notes); ?></p>
                            <?php endif; ?>
                        </div>
                    </div>

                    <?php if($temporaryUpload->status === 'pending'): ?>
                    ...
                    <?php elseif($temporaryUpload->status === 'processing'): ?>
                        <div class="alert alert-warning">
                            <i class="bi bi-clock me-2"></i>
                            Your file is currently being processed. Please wait...
                        </div>
                    <?php elseif($temporaryUpload->status === 'failed'): ?>
            ...
            <!-- Processing Progress -->
            <?php if($temporaryUpload->status === 'processing'): ?>
                <div class="card mb-4">
                    <div class="card-body">
            ...
            <!-- Items from Upload -->
            <?php if($temporaryUpload->associates && $temporaryUpload->associates->count() > 0): ?>
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h5 class="mb-0">Items from Upload (<?php echo e($temporaryUpload->associates->count()); ?>)</h5>
                        <div class="btn-group">
                    ...
                    <div class="card-body">
                        ...
                        <div id="debugInfo" class="mt-3" style="display: none;">
                            <div class="alert alert-info">
                                ...
                                File: <?php echo e($temporaryUpload->original_filename ?? 'Unknown'); ?><br>
                                <?php
                                    $firstAssociate = $temporaryUpload->associates->first();
                                    $serviceType = $firstAssociate ? $firstAssociate->service_type : 'register_post';
                                ?>
                                Service Type: <?php echo e(ucfirst(str_replace('_', ' ', $serviceType))); ?><br>
                                Upload Time: <?php echo e($temporaryUpload->created_at->format('M d, Y H:i:s')); ?>
            ...
            <?php if($temporaryUpload->associates && $temporaryUpload->associates->count() > 0): ?>
                    <div class="card-body">
                        ...
                        <?php if($missingDataCount > 0): ?>
                            <div class="alert alert-warning mb-3">
                                <i class="bi bi-exclamation-triangle me-2"></i>
                                <strong>Data Issue Detected:</strong> <?php echo e($missingDataCount); ?> item(s) are missing receiver name or address information.
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    <?php $__currentLoopData = $temporaryUpload->associates; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $associate): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <tr>
                                            <td>
                                                <div class="form-check">
                                                    <input class="form-check-input item-checkbox" type="checkbox" value="<?php echo e($associate->id); ?>" onchange="updateDeleteButton()">
                                                </div>
                                            </td>
                                            <td><?php echo e($index + 1); ?></td>
                                            <td>
                                                <?php if($associate->receiver_name): ?>
                                                    <?php echo e($associate->receiver_name); ?>
                                            ...
                                            <td>
                                                <?php if($associate->receiver_address): ?>
                                                    <?php echo e(Str::limit($associate->receiver_address, 50)); ?>
                                            ...
                                            <td>
                                                ...
                                                ?>
                                                <span class="badge bg-primary"><?php echo e($typeLabels[$associate->service_type] ?? $associate->service_type); ?></span>
                                            </td>
                                            <td>LKR <?php echo e(number_format($associate->amount, 2)); ?></td>
                                            <td>
                                                <?php if($associate->weight): ?>
                                                    <?php echo e(number_format($associate->weight)); ?>g
                                                <?php else: ?>
                                                    -
                                                <?php endif; ?>
                                            </td>
                                            <td>LKR <?php echo e(number_format($associate->postage, 2)); ?></td>
                                            <td>
                                                <?php if($associate->barcode): ?>
                                                    <code class="text-primary"><?php echo e($associate->barcode); ?></code>
                                                <?php else: ?>
                                                    <span class="text-muted fst-italic">Not provided</span>
                                                <?php endif; ?>
                                            </td>
                                            <td>
                                                <?php switch($associate->status):
                                                    case ('accept'): ?>
                                            ...
                                            <td>
                                                <button class="btn btn-sm btn-outline-primary" onclick="editItem(<?php echo e($associate->id); ?>, '<?php echo e($associate->receiver_name); ?>', '<?php echo e($associate->receiver_address); ?>', '<?php echo e($associate->item_value); ?>', '<?php echo e($associate->service_type); ?>', '<?php echo e($associate->weight); ?>', '<?php echo e($associate->amount); ?>', '<?php echo e($associate->barcode); ?>')" data-bs-toggle="modal" data-bs-target="#editItemModal">
                                                    <i class="bi bi-pencil"></i>
                                                </button>
                                                <button class="btn btn-sm btn-outline-danger" onclick="deleteItem(<?php echo e($associate->id); ?>)">
                                                    <i class="bi bi-trash"></i>
                                                </button>
                                            </td>
                                        </tr>
                            ...
                            <div class="col-md-12">
                                <div class="card bg-light">
                                    <div class="card-body">
                                        ...
                                        <div class="row">
                                            <div class="col-md-3">
                                                <strong>Total Items:</strong> <?php echo e($temporaryUpload->associates->count()); ?>

                                            </div>
                                            <div class="col-md-3">
                                                <strong>Total Amount:</strong> LKR <?php echo e(number_format($temporaryUpload->associates->sum('amount'), 2)); ?>

                                            </div>
                                            <div class="col-md-3">
                                                <strong>Total Postage:</strong> LKR <?php echo e(number_format($temporaryUpload->associates->sum('postage'), 2)); ?>

                                            </div>
                                            <div class="col-md-3">
                                                <strong>Total Commission:</strong> LKR <?php echo e(number_format($temporaryUpload->associates->sum('commission'), 2)); ?>
            ...
            <div class="row mt-4">
                <div class="col-md-12 text-center">
                    <?php if($temporaryUpload->associates && $temporaryUpload->associates->count() > 0 && $temporaryUpload->status !== 'submitted'): ?>
                        <form method="POST" action="<?php echo e(route('customer.services.submit-bulk-to-pm', $temporaryUpload->id)); ?>" style="display: inline-block;">
                            <?php echo csrf_field(); ?>
                    ...
                    <a href="<?php echo e(route('customer.services.items')); ?>" class="btn btn-outline-secondary">
                        <i class="bi bi-list-ul me-2"></i>View All Items
                    </a>
                    <?php if($temporaryUpload->status === 'completed'): ?>
...
function deleteItem(id) {
    if (confirm('Are you sure you want to delete this item?')) {
        fetch(`<?php echo e(route('customer.services.delete-bulk-item', ':id')); ?>`.replace(':id', id), {
            method: 'DELETE',
            headers: {
...
document.getElementById('editItemForm').addEventListener('submit', function(e) {
    ...
    fetch(`<?php echo e(route('customer.services.update-bulk-item', ':id')); ?>`.replace(':id', currentEditId), {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>',
...
function deleteSelected() {
    ...
    if (confirm(`Are you sure you want to delete ${checkedBoxes.length} selected item(s)?`)) {
        ...
        const deletePromises = selectedIds.map(id => {
            return fetch(`<?php echo e(route('customer.services.delete-bulk-item', ':id')); ?>`.replace(':id', id), {
                method: 'DELETE',
                headers: {
                    'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>',
                    'Content-Type': 'application/json',
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\User\Desktop\NEW_ONE-main\resources\views/customer/services/bulk-status.blade.php ENDPATH**/ ?>