<?php $__env->startSection('title', 'Create COD Item'); ?>
        ...
        <div class="col-12">
            ...
            <div class="card border-0 rounded-top-0 shadow">
                <div class="card-body p-4">
                    <form method="POST" action="<?php echo e(route('pm.single-item.store-cod')); ?>">
                        <?php echo csrf_field(); ?>
                    ...
                    <h5 class="mb-0">
                        ...
                        <div class="row mb-3">
                            <div class="col-md-4">
                                ...
                                <small class="text-muted">Weight in grams</small>
                            </div>
                            <div class="col-md-4">
                        ...
                        <div class="alert alert-warning" id="totalAmountAlert" style="display: none;">
                            <div class="row">
                                <div class="col-md-6">
                                    <strong>COD Amount:</strong> <span id="codAmountDisplay">LKR 0.00</span>
                                </div>
                                <div class="col-md-6">
                                    <strong>Postage:</strong> <span id="postageAmountDisplay">LKR 0.00</span>
                                </div>
                            </div>
                            <hr class="my-2">
                            <div class="text-center">
                                <strong class="fs-5">Total to Collect: <span id="totalAmountDisplay" class="text-danger">LKR 0.00</span></strong>
                            </div>
                        </div>
                            ...
                            <div class="col-12">
                                <div class="d-flex justify-content-between">
                                    ...
                                    <button type="submit" class="btn btn-warning btn-lg" id="submitBtn">
                                        <i class="bi bi-check-circle"></i> Create COD Item
                                    </button>
                                </div>
                            </div>
                        </div>
...
$(document).ready(function() {
    ...
    $('#generateBarcode').click(function() {
        const barcode = 'COD' + Date.now() + Math.floor(Math.random() * 1000);
        $('#barcode').val(barcode);
    });
    ...
    // Update total when COD amount changes
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    $('#postage_display').data('amount', response.postage);
    ...
    function updateTotalAmount() {
        const codAmount = parseFloat($('#amount').val()) || 0;
        const postageAmount = parseFloat($('#postage_display').data('amount')) || 0;
        const totalAmount = codAmount + postageAmount;

        if (codAmount > 0 || postageAmount > 0) {
            $('#codAmountDisplay').text('LKR ' + codAmount.toFixed(2));
            $('#postageAmountDisplay').text('LKR ' + postageAmount.toFixed(2));
            $('#totalAmountDisplay').text('LKR ' + totalAmount.toFixed(2));
...
<?php echo $__env->make('layouts.modern-pm', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\User\Desktop\NEW_ONE-main\resources\views/pm/single-item/cod-form.blade.php ENDPATH**/ ?>