...
@section('title', 'Change Dispatch Location - Step 3')
            ...
            <div class="card shadow-sm border-success">
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <p><strong>Current Destination:</strong> <span class="badge badge-danger">{{ $dispatch->destinationOffice->name }}</span></p>
        ...
        <div class="col-md-8">
            <div class="card shadow">
                ...
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ route('pm.dispatch.update-location') }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-group">
                            ...
                            <select name="new_destination_office"
                                    ...
                                    class="form-control form-control-lg @error('new_destination_office') is-invalid @enderror"
                                    required>
                                <option value="">Select new destination office...</option>
                                @foreach($deliveryOffices as $office)
                                    <option value="{{ $office->id }}"
                                            @if($office->id == $dispatch->destination_office) disabled class="text-muted" @endif
                                            {{ old('new_destination_office') == $office->id ? 'selected' : '' }}>
                                        {{ $office->name }}
                                        @if($office->id == $dispatch->destination_office) (Current Destination) @endif
                                    </option>
                                @endforeach
                            </select>
                            @error('new_destination_office')
                        ...
                        <div class="alert alert-warning border-warning">
                            <div class="d-flex align-items-center">
                                ...
                                <div>
                                    ...
                                    <p class="mb-0">Are you sure you want to change the destination office for dispatch <strong>{{ $dispatch->necklabel }}</strong>? This action will affect all {{ $dispatch->dispatchAssociates->count() }} items in this dispatch and cannot be undone.</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="text-center">
                            ...
                            <a href="{{ route('pm.dispatch.index') }}" class="btn btn-secondary btn-lg ml-2">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <ul class="list-unstyled mb-3">
                                <li><i class="fas fa-tag text-muted"></i> <strong>Necklabel:</strong> {{ $dispatch->necklabel }}</li>
                                <li><i class="fas fa-file-alt text-muted"></i> <strong>Manifest ID:</strong> {{ $dispatch->manifest_id }}</li>
                                <li><i class="fas fa-building text-muted"></i> <strong>Current Destination:</strong> {{ $dispatch->destinationOffice->name }}</li>
                                <li><i class="fas fa-boxes text-muted"></i> <strong>Items Count:</strong> {{ $dispatch->dispatchAssociates->count() }}</li>
                            </ul>
                        </div>
                        <div class="col-md-6">
