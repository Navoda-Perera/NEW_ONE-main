...
<head>
    ...
    <title>ITEM MANIFEST - {{ $dispatch->manifest_id }}</title>
    <style>
        @page {
            size: A4;
            margin: 20mm;
...
<body>
    <div class="manifest-container">
        ...
        <div class="manifest-info">
            <table>
                <tr>
                    <td><strong>User:</strong> {{ $dispatch->creator->name }}</td>
                    <td><strong>List Serial No:</strong> {{ $dispatch->manifest_id }}</td>
                </tr>
                <tr>
                    <td><strong>Office:</strong> {{ $dispatch->location->name }}</td>
                    <td><strong>Neck Label:</strong> {{ $dispatch->necklabel }}</td>
                </tr>
                <tr>
                    <td><strong>Date:</strong> {{ $dispatch->created_at->format('Y-m-d H:i:s') }}</td>
                    <td><strong>Number of Item:</strong> {{ $dispatchedItems->count() }}</td>
                </tr>
                <tr>
                    <td><strong>To Office:</strong> {{ $dispatch->destinationOffice->name }}</td>
                    <td></td>
                </tr>
            </table>
        </div>
        ...
        <div class="footer-info">
            ...
            <ul style="margin-left: 20px;">
                <li>Total Items: {{ $dispatchedItems->count() }}</li>
                <li>Total Value: LKR {{ number_format($dispatchedItems->sum('item.amount'), 2) }}</li>
                <li>Generated: {{ now()->format('d/m/Y H:i:s') }}</li>
            </ul>
        </div>
