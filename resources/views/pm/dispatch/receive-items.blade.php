...
@section('title', 'Receive Items - ' . $dispatch->necklabel)
...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <div class="d-flex">
            <a href="{{ route('pm.dispatch.receive-by-necklabel') }}" class="btn btn-sm btn-secondary shadow-sm">
        ...
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-md-3">
                    ...
                    <div>{{ $dispatch->manifest_id }}</div>
                </div>
                <div class="col-md-3">
                    <strong class="text-success">From Location:</strong>
                    <div>{{ $dispatch->location->name }}</div>
                </div>
                <div class="col-md-3">
                    <strong class="text-success">Total Items:</strong>
                    <div><span class="badge badge-info">{{ $dispatchAssociates->count() }}</span> items to receive</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <form action="{{ route('pm.dispatch.mark-items-received') }}" method="POST" id="receiveItemsForm">
        @csrf
            ...
            <div class="card-body p-0">
                @if($dispatchAssociates->count() > 0)
                    <div class="table-responsive">
                        <table class="table table-hover mb-0">
                            ...
                            <tbody>
                                @foreach($dispatchAssociates as $index => $associate)
                                    <tr class="item-row" data-item-id="{{ $associate->item_id }}">
                                        <td class="text-center">
                                            <div class="form-check">
                                                <input
                                                    type="checkbox"
                                                    name="item_ids[]"
                                                    value="{{ $associate->item_id }}"
                                                    class="form-check-input item-checkbox"
                                                    id="item_{{ $associate->item_id }}"
                                                >
                                                <label class="form-check-label" for="item_{{ $associate->item_id }}"></label>
                                            </div>
                                        </td>
                                        ...
                                        <td>
                                            <code class="bg-light px-2 py-1 rounded text-danger">{{ $associate->item->barcode }}</code>
                                        </td>
                                        <td class="text-center">
                                            <span class="badge badge-warning">{{ ucfirst($associate->status) }}</span>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                ...
                @else
                    <div class="text-center p-5">
                        ...
                        <a href="{{ route('pm.dispatch.receive-by-necklabel') }}" class="btn btn-primary">
